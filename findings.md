...
## 2026-07-25 763898834 群意图错误根因

- 只读会话状态确认错误发生在两轮粤语餐饮闲聊；入口、白名单和回复发送均正常，错误不在 QQ 唤醒层。
- 新 Gateway 没有独立意图分类器。`GatewayAgent.#handleSerial()` 对每个有效事件第一步固定执行 Top-16 混合 RAG，并把所有超过 `RAG_EVIDENCE_THRESHOLD` 的结果写入模型提示。
- 当前阈值为 0.03，但 Qdrant RRF 返回的是融合排名分，不是可校准的语义相关度。餐饮闲聊的无关论文片段仍得到 0.5、0.333、0.25 等分数，因此全部越过阈值。
- 正常 CPU 问题的正确结果为 0.7/0.5208，但无关餐饮结果也能达到 0.5；单纯提高一个全局阈值会同时漏掉部分有效本地证据，不能作为可靠修复。
- 系统提示写明“回答前已经执行了本地知识检索”，而注入证据与问题无关，模型于是把普通闲聊回答成“知识库没有资料”。这不是 DeepSeek 自身不会识别意图，而是编排层从未让它选择响应模式。
- 用同一 `deepseek-v4-pro` 做隔离结构化分类时，该餐饮请求得到 `chat_creative/0.95`。128 completion tokens 全被推理消耗而返回空正文；512 上限时共使用 211 completion tokens，其中 194 为 reasoning tokens。后续若恢复路由器，应关闭分类器思考或使用低延迟非推理模型，不能沿用 128 token 上限。
- 诊断阶段未修改生产；随后按用户授权重新建立“响应模式路由”，同时避免恢复旧架构中“路由失败即禁止 RAG”的单点硬门。
- 修复后的 Gateway 先以隔离 JSON 路由选择响应模式：闲聊无工具；外部事实仅网页搜索；本地知识自动 RAG；技术问题自动 RAG 并保留网页深挖；路由异常不注入证据但同时保留两种工具。
- 首轮 canary 证明“我的服务器 CPU”容易被误认为实时状态；最终契约明确硬件清单与用户专属资料属于 `local_knowledge`。`local_runtime` 不自动注入文档，但保留按需 RAG 工具并要求不得把文档冒充实时状态。
- 最终远端结果：同一粤语餐饮请求约 2.1 秒、无知识库措辞/脚注；CPU 请求约 2.5 秒、命中 9470C 且来源非 unknown；外部事实约 20.4 秒、含 URL 且无本地来源。

## 2026-07-26 管理员私聊循环与群引用语境根因

- 生产 NapCat 活动 WebSocket Client 为 `reportSelfMessage=true`，AstrBot 的 `ignore_bot_self_message=false`；该组合是号主人工接管的既有要求，不能简单关闭自身消息上报。
- 管理员私聊出站消息被再次上报为同一好友会话的空消息。七分钟内 Gateway 保存 22 个回复决策，私聊记忆增长到 24 个交换，其中多数 user 文本为空；用户发出停止指令后仍继续出现空入站与回复。
- 该私聊回流在 AstrBot 中显示为好友账号发送者，而不是 `sender_id == self_id`，所以只加自身 ID 判断不能解决生产故障。可靠边界是：精确匹配出站 message_id、插件拒绝无可处理文本、Gateway 拒绝空事件。
- 群 902811542 的目标消息本身只有一个短省略问句，但 OneBot Reply 组件已包含被引用消息正文。插件当前只复制 `reply_to_sender_id`，丢弃 `Reply.message_str`。
- 真实引用正文明确指向 Pi/Gateway 技术讨论；Gateway 路由器却只接收当前文本，且会话记忆在分类之后才读取，因此模型无法恢复指代并错误使用本地知识路径。
- 最小修复是新增有界 `reply_context` 并在路由、检索与最终回答中复用；近期记忆作为次级上下文，引用正文优先。所有上下文继续按不可信数据隔离。

## 2026-07-26 引用追问与自动 RAG 的生产边界

- 仅把引用正文送入分类器还不够；路由提示必须明确先解析引用中的主题，再判断短评价问句。技术工具、模型、算法、服务或集成上的省略式评价应属于 `technical_concept`。
