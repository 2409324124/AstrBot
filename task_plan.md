...
- [ ] 完成本地门禁、提交并推送现有 feature 分支
- [ ] 备份远端源码/配置/SQLite，清理空输入污染并灰度部署 AstrBot 与 Gateway
- [ ] 验证管理员私聊单次回复、群 902811542 引用追问及号主人工接管无回归
- **状态：** in_progress
...
| 响应模式路由控制自动证据注入，但不封死工具 | 闲聊/外部事实不注入本地 RAG；本地知识/技术自动检索；路由失败仍开放 RAG 与网页工具，避免旧架构的硬门。 |
