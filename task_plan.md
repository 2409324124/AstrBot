...
- [x] 远端旁路部署、备份、一次性切换白名单会话并验证回滚开关
- **状态：** in_progress

### 阶段 12：763898834 群意图错误诊断

- [x] 只读定位该群最近事件、Gateway 决策与回复时间线
- [x] 区分唤醒判断、自动 RAG、工具调用、会话记忆和模型生成边界
- [x] 用同一输入建立可重复的最小复现，并与正常事件对比
- [x] 形成单一根因假设；未经证实不修改生产
- [x] 用户授权后完成 RED→GREEN、canary、正式部署与三路径回归
- **状态：** completed

### 阶段 13：管理员私聊循环与引用语境修复

- [x] RED→GREEN：空白管理员私聊在插件与 Gateway 两层拒绝，不调用路由/RAG/LLM
- [x] RED→GREEN：按精确出站 message_id 丢弃私聊自身回流，不用文本指纹误吞用户消息
- [x] RED→GREEN：QQ 引用正文以有界 `reply_context` 进入 Gateway、路由、RAG 与最终提示
- [x] RED→GREEN：路由使用有界近期记忆，省略式追问不再脱离上下文
- [x] 增加不含正文的路由、RAG、工具和耗时审计
- [x] 完成本地门禁、提交并推送现有 feature 分支
- [x] 备份远端源码/配置/SQLite，清理空输入污染并灰度部署 AstrBot 与 Gateway
- [x] 合成验证空事件拒绝、引用追问正确路由、按需 RAG/Web 与无关证据隔离
...
| 响应模式路由控制自动证据注入，但不封死工具 | 仅明确本地知识请求自动注入 RAG；技术问题由模型按需调用 RAG/Web；路由失败仍开放两种工具，避免旧架构的硬门与低相关证据污染。 |
...
| 两次只读 SQLite Node 命令的正则括号错误 | 2 | 均未修改数据；去掉非必要 URL 正则后成功读取目标交换。 |
| 一次 Gateway 路由探针的 shell 反引号转义错误 | 1 | 未调用模型或修改服务；已有持久状态和源码边界足以证明根因，不重复该探针。 |
